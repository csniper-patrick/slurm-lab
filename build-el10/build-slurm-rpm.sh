#!/bin/bash -x
# This script builds Slurm RPM packages for Enterprise Linux 10 (RHEL/Rocky 10).
...
dnf -y install munge munge-devel mariadb mariadb-devel gtk3 gtk3-devel llhttp llhttp-devel json-c json-c-devel libyaml libyaml-devel wget python3 readline-devel pam-devel perl-ExtUtils-MakeMaker perl-devel perl-JSON-PP createrepo_c hdf5 hdf5-devel man2html man2html-core pam pam-devel freeipmi freeipmi-devel numactl numactl-devel pmix pmix-devel hwloc hwloc-devel lua lua-devel ucx ucx-devel "libjwt < 3.0.0" "libjwt-devel < 3.0.0"
...
# Get version and release from the spec file
ver=$(rpmspec -q --qf '%{version}\n' /usr/local/src/slurm/slurm.spec | head -n 1)
rel=$(rpmspec -q --qf '%{release}\n' /usr/local/src/slurm/slurm.spec | head -n 1 | cut -d. -f1)
if [[ ${rel} == 1 ]]; then 
...
python3 -m pip install --break-system-packages pypandoc
make html
